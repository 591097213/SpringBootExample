...
public class InitRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Collection[] collections = initAccountsAndPnoNums(RandomUtils.getRandomNumber(1, 3));
            ...
            user.setEmail(email);
            user.setAge(age);
            user.setAccounts(collections[0]);
            user.setAddresses(addresses);
            user.setApps(apps);
            user.setPhoNums(collections[1]);

            userRepository.save(user);
            Collection<User> users = new ArrayList<User>();
            users.add(user);

            for (Account tempAccount : (ArrayList<Account>) collections[0]) {
            ...
            for (PhoNum tempPhoNum : (ArrayList<PhoNum>) collections[1]) {
                tempPhoNum.setUser(user);
                phoNumRepository.save(tempPhoNum);
            }
    ...
    public Collection[] initAccountsAndPnoNums(int num) {

        Collection<Account> accounts = new ArrayList<Account>();
        Collection<PhoNum> phoNums = new ArrayList<PhoNum>();
        ...
        Collection[] collections = {accounts, phoNums};
        return collections;
    ...
    // public Collection<PhoNum> initPhoNUms(int num) {
    // for (int i = 0; i < num; i++) {
